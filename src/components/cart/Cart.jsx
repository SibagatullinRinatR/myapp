...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://63985bc6044fa481d69a6e2b.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://63985bc6044fa481d69a6e2b.mockapi.io/cart', obj)
    ...
    const onAddFav = async (obj)=>{
        ...
        try{
            const findFavorites = props.favorites.find(objFav => objFav.myId === obj.myId)
             if(findFavorites){
            axios.delete(`https://63985bc6044fa481d69a6e2b.mockapi.io/favorites/${findFavorites.id}`)
            props.setFavorites((over) => over.filter(item => item.myId !== obj.myId))

        }
        else{
          const {data} = await  axios.post('https://63985bc6044fa481d69a6e2b.mockapi.io/favorites', obj)
